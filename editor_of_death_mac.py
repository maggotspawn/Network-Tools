...
# HARDCODED TOOL PATHS FOR PYINSTALLER - THESE ARE CONFIRMED WORKING
PYRIGHT_EXECUTABLE = "/Library/Frameworks/Python.framework/Versions/3.14/bin/pyright"
RUFF_EXECUTABLE = "/Library/Frameworks/Python.framework/Versions/3.14/bin/ruff"

from PyQt6.QtCore import Qt, QRect, QSize, QSettings, QPoint, QMimeData, QTimer
...
class EditorMain(QMainWindow):
    def __init__(self):
        ...
        self.run_with_syntax_errors = self.settings.value("run_with_syntax_errors", False, type=bool)
    ...
    def _create_actions(self):
        ...
        self.run_with_errors_act = QAction("Run Even With Syntax Errors", self, checkable=True, triggered=self.toggle_run_with_errors)
        self.run_with_errors_act.setChecked(self.run_with_syntax_errors)
    ...
    def _create_menus(self):
        ...
        tools.addAction(self.run_with_errors_act)
        tools.addSeparator()
    ...
    # ---------- Run with errors toggle ----------
    def toggle_run_with_errors(self, checked: bool):
        """Toggle the run with syntax errors setting"""
        self.run_with_syntax_errors = checked
        self.settings.setValue("run_with_syntax_errors", checked)
        status_msg = "Will now run Python scripts even with syntax errors" if checked else "Will check syntax before running Python scripts"
    ...
    def _syntax_check_python(self, text: str, filepath: str = None):
        """
        Aggressive fix for pyright and ruff in PyInstaller bundles.
        Tries multiple methods to make subprocess calls work.
        """
        issues = []
        
        # Try every possible method to find tools
        def find_tool_aggressive(tool_name):
            """Try every possible method to find a tool"""
            import shutil
            
            # PRIORITY 1: Use hardcoded paths that we KNOW work
            if tool_name == 'pyright' and os.path.exists(PYRIGHT_EXECUTABLE):
                return PYRIGHT_EXECUTABLE
            if tool_name == 'ruff' and os.path.exists(RUFF_EXECUTABLE):
                return RUFF_EXECUTABLE
            
            # PRIORITY: Check known absolute paths first (based on test results)
            if platform.system() == 'Darwin':
                # These are the EXACT paths where pyright and ruff are installed on this system
                known_paths = {
                    'pyright': '/Library/Frameworks/Python.framework/Versions/3.14/bin/pyright',
                    'ruff': '/Library/Frameworks/Python.framework/Versions/3.14/bin/ruff'
                }
                
                if tool_name in known_paths:
                    known_path = known_paths[tool_name]
                    if os.path.exists(known_path) and os.access(known_path, os.X_OK):
                        print(f"Found {tool_name} at known path: {known_path}", file=sys.stderr)
                        return known_path
            
            # Method 1: shutil.which (most reliable for general cases)
            path = shutil.which(tool_name)
            if path and os.path.exists(path):
                return path
            
            # Special handling for pyright installed via npm
            if tool_name == 'pyright':
                # Try running pyright directly through npx
                try:
                    result = subprocess.run(['npx', '--no-install', 'pyright', '--version'], 
                                          capture_output=True, text=True, timeout=5)
                    if result.returncode == 0 and 'pyright' in result.stdout.lower():
                        print(f"Found pyright via npx", file=sys.stderr)
                        return 'npx:pyright'  # Special marker for npx execution
                except:
                    pass
            
            # Method 2: subprocess which/where
            try:
                cmd = ['which', tool_name] if platform.system() != 'Windows' else ['where', tool_name]
            ...
            # Method 3: Direct path search
            home = Path.home()
            search_paths = []
            
            if platform.system() == 'Darwin':
                search_paths = [
                    # CRITICAL: Add Python 3.14 framework path FIRST (where pyright is actually installed)
                    f'/Library/Frameworks/Python.framework/Versions/3.14/bin/{tool_name}',
                    f'/Library/Frameworks/Python.framework/Versions/Current/bin/{tool_name}',
                    # Then check other common locations
                    f'/opt/homebrew/bin/{tool_name}',
                    f'/usr/local/bin/{tool_name}',
                    f'{home}/.local/bin/{tool_name}',
                    f'{home}/Library/Python/3.11/bin/{tool_name}',
                    f'{home}/Library/Python/3.12/bin/{tool_name}',
                    f'{home}/Library/Python/3.13/bin/{tool_name}',
                    f'{home}/Library/Python/3.14/bin/{tool_name}',
                ]
            elif platform.system() == 'Windows':
                import glob
                for pattern in [
                    f'{home}\\AppData\\Local\\Programs\\Python\\Python*\\Scripts\\{tool_name}.exe',
                    f'{home}\\AppData\\Roaming\\Python\\Python*\\Scripts\\{tool_name}.exe',
                ]:
                    search_paths.extend(glob.glob(pattern))
            
            for path in search_paths:
                if os.path.exists(path):
                    print(f"Found {tool_name} at: {path}", file=sys.stderr)
                    if os.access(path, os.X_OK):
                        print(f"  ✓ Executable, using this path", file=sys.stderr)
                        return path
                    else:
                        print(f"  ✗ Not executable", file=sys.stderr)
            
            return None
        
        # Find the tools
        pyright_path = find_tool_aggressive('pyright')
        ruff_path = find_tool_aggressive('ruff')
        
        if not pyright_path and not ruff_path:
            QMessageBox.warning(self, "Syntax Checkers Not Found", 
                "Could not find pyright or ruff.\n\n"
                "Please install with:\n"
        ...
        try:
            # Try multiple subprocess execution methods
            def run_tool_aggressive(tool_path, tool_name, filepath):
                """Try multiple methods to run a tool"""
                
                # Special handling for npx pyright
                if tool_path == 'npx:pyright':
                    env = os.environ.copy()
                    if platform.system() == 'Darwin':
                    ...
                    try:
                        print(f"Running pyright via npx", file=sys.stderr)
                        result = subprocess.run(
                            ['npx', '--no-install', 'pyright', '--outputjson', filepath],
                            capture_output=True,
                            text=True,
                            timeout=30,  # npx can be slower
                            env=env,
                        ...
                        if result.stdout:
                            ...
                            print(f"npx pyright stderr: {result.stderr[:200]}", file=sys.stderr)
                    except Exception as e:
                        print(f"npx pyright failed: {e}", file=sys.stderr)
                    return None, None
                
                # Build command for normal tools
                if tool_name == 'pyright':
                    cmd_args = ['--outputjson', filepath]
                else:  # ruff
                    cmd_args = ['check', '--output-format=json', filepath]
                
                # Setup environment - CRITICAL for pyright which needs Node.js
                env = os.environ.copy()
                if platform.system() == 'Darwin':
                    # Comprehensive PATH for macOS PyInstaller bundles
                    # CRITICAL: Pyright needs Node.js, so add Node.js paths
                    path_components = [
                        # Python paths (for both pyright and ruff)
                        '/Library/Frameworks/Python.framework/Versions/3.14/bin',
                        '/Library/Frameworks/Python.framework/Versions/3.13/bin',
                        '/Library/Frameworks/Python.framework/Versions/Current/bin',
                        # Node.js paths (CRITICAL for pyright)
                        '/opt/homebrew/bin',  # Homebrew Node.js
                        '/usr/local/bin',     # Traditional Node.js location
                        str(Path.home() / '.nvm/versions/node/*/bin'),  # nvm installations
                    ...
                    # Expand glob patterns and filter valid paths
                    import glob
                    expanded_paths = []
                    ...
                    valid_paths = [p for p in expanded_paths if os.path.exists(p)]
                    env['PATH'] = ':'.join(valid_paths) + ':' + existing_path
                    print(f"PATH for {tool_name}: {env['PATH'][:200]}...", file=sys.stderr)
                
                elif platform.system() == 'Windows':
                    # Windows PATH setup
                    path_components = [
                        str(Path.home() / 'AppData/Local/Programs/Python/Python314/Scripts'),
                        str(Path.home() / 'AppData/Local/Programs/Python/Python313/Scripts'),
                        str(Path.home() / 'AppData/Roaming/npm'),  # npm global packages
                        'C:/Program Files/nodejs',  # Node.js
                        str(Path.home() / 'AppData/Roaming/Python/Python314/Scripts'),
                    ]
                    existing_path = env.get('PATH', '')
                    valid_paths = [p for p in path_components if os.path.exists(p)]
                    env['PATH'] = ';'.join(valid_paths) + ';' + existing_path
                
                # Method 1: Direct execution (works for both pyright and ruff)
                try:
                    full_cmd = [tool_path] + cmd_args
                    ...
                    print(f"{tool_name} return code: {result.returncode}", file=sys.stderr)
                    
                    # For pyright, returncode might be non-zero even with valid output
                    if result.stdout:
                        print(f"✓ Got {tool_name} output: {len(result.stdout)} bytes", file=sys.stderr)
                        return result.stdout, result.stderr
                    elif result.stderr:
                        print(f"{tool_name} stderr: {result.stderr[:500]}", file=sys.stderr)
                        # Check if stderr contains JSON output (some tools output to stderr)
                        if result.stderr.strip().startswith('{'):
                            return result.stderr, None
                except Exception as e:
                    print(f"✗ Direct execution failed for {tool_name}: {e}", file=sys.stderr)
                
                # Method 2: Shell execution (fallback)
                try:
                    if platform.system() == 'Windows':
                        shell_cmd = f'"{tool_path}" {" ".join(cmd_args)}'
                    else:
                        shell_cmd = f"'{tool_path}' {' '.join(cmd_args)}"
                    
                    print(f"Trying shell execution: {shell_cmd[:100]}", file=sys.stderr)
                    result = subprocess.run(
                        shell_cmd,
                        shell=True,
                        capture_output=True,
                    ...
                    if result.stdout:
                        print(f"✓ Shell execution succeeded for {tool_name}", file=sys.stderr)
                        return result.stdout, result.stderr
                except Exception as e:
                    print(f"✗ Shell execution failed for {tool_name}: {e}", file=sys.stderr)
                
                return None, None
            
            # Run pyright
            if pyright_path:
                print(f"Running pyright: {pyright_path}", file=sys.stderr)
                stdout, stderr = run_tool_aggressive(pyright_path, 'pyright', filepath)
                if stdout:
                    print(f"Pyright returned {len(stdout)} bytes of output", file=sys.stderr)
                    try:
                        import json
                        data = json.loads(stdout)
                        diags = data.get('generalDiagnostics', [])
                        print(f"Pyright found {len(diags)} diagnostics", file=sys.stderr)
                        for diag in diags:
                            if diag.get('file', '').endswith(os.path.basename(filepath)):
                                line = diag.get('range', {}).get('start', {}).get('line', 0) + 1
                                col = diag.get('range', {}).get('start', {}).get('character', 0) + 1
                                severity = diag.get('severity', 'error')
                                message = diag.get('message', 'Unknown error')
                                issues.append((line, col, f"[pyright:{severity}] {message}"))
                    except Exception as e:
                        print(f"Failed to parse pyright output: {e}", file=sys.stderr)
            
            # Run ruff
            if ruff_path:
                print(f"Running ruff: {ruff_path}", file=sys.stderr)
                stdout, stderr = run_tool_aggressive(ruff_path, 'ruff', filepath)
                if stdout:
                    print(f"Ruff returned {len(stdout)} bytes of output", file=sys.stderr)
                    try:
                        import json
                        data = json.loads(stdout)
                        print(f"Ruff found {len(data)} issues", file=sys.stderr)
                        for issue in data:
                            line = issue.get('location', {}).get('row', 1)
                            col = issue.get('location', {}).get('column', 1)
                            message = issue.get('message', 'Unknown issue')
                            code = issue.get('code', '')
                            issues.append((line, col, f"[ruff:{code}] {message}"))
                    except Exception as e:
                        print(f"Failed to parse ruff output: {e}", file=sys.stderr)
        ...
        # Debug output if no issues found
        if not issues and (pyright_path or ruff_path):
            print(f"No issues found. Tools: pyright={pyright_path}, ruff={ruff_path}", file=sys.stderr)
    ...
    def run_selected_in_console(self):
        ...
        # Check for syntax errors before running (unless disabled)
        if not self.run_with_syntax_errors:
            self.status.showMessage("Checking syntax before running...", 2000)
            QApplication.processEvents()  # Update UI immediately
        
            issues = self._syntax_check_python(code, getattr(ed, 'path', None))
        
            # Filter for actual errors (not warnings)
            errors = [issue for issue in issues if 'error' in issue[2].lower()]
        
            if errors:
            # Show error count in status bar
                error_msg = f"⚠️ Cannot run: {len(errors)} syntax error{'s' if len(errors) > 1 else ''} found"
                self.status.showMessage(error_msg, 10000)  # Show for 10 seconds
                self.status.setStyleSheet("QStatusBar { background-color: #4a0000; color: #ff6666; }")
            
            # Highlight the error lines
                self._highlight_issue_lines(ed, errors, QColor(255, 100, 100, 100))
            
            # Go to first error
                if errors:
                    line, col, msg = errors[0]
                    self._goto_line_in_editor(ed, line, col)
            
            # Show detailed error dialog
                error_text = f"Cannot run {fname} due to syntax errors:\n\n"
                for line, col, msg in errors[:5]:  # Show first 5 errors
                    error_text += f"Line {line}: {msg}\n"
                if len(errors) > 5:
                    error_text += f"\n... and {len(errors) - 5} more errors"
            
                
                error_text += "\n\nTo run anyway, enable 'Run Even With Syntax Errors' in Tools menu."
                
                QMessageBox.warning(self, "Syntax Errors Prevent Execution", error_text)
            
            # Reset status bar style after a delay
                QTimer.singleShot(10000, lambda: self.status.setStyleSheet(""))
                return
        
            # If we have warnings but no errors, show them in status bar but continue
            if issues:
                warning_msg = f"ℹ️ Running with {len(issues)} warning{'s' if len(issues) > 1 else ''}"
                self.status.showMessage(warning_msg, 5000)
                self.status.setStyleSheet("QStatusBar { background-color: #4a4a00; color: #ffff66; }")
                QTimer.singleShot(5000, lambda: self.status.setStyleSheet(""))
            else:
                self.status.showMessage("✓ No syntax errors found", 2000)
        
            # Check if Python is available and actually works (not just a Windows Store stub)
        else:
            # User has disabled syntax checking
            self.status.showMessage("⚠️ Running without syntax check (enabled in settings)", 3000)
        ...
        try:
            ...
            # Create clean environment to prevent splash screen distortion
            clean_env = os.environ.copy()
            # Remove Qt-related environment variables that might affect child processes
            qt_vars = ['QT_PLUGIN_PATH', 'QT_QPA_PLATFORM_PLUGIN_PATH', 'QT_SCALE_FACTOR',
                      'QT_AUTO_SCREEN_SCALE_FACTOR', 'QT_SCREEN_SCALE_FACTORS']
            for var in qt_vars:
                clean_env.pop(var, None)
            
            # Add PYTHONUNBUFFERED to ensure output is displayed immediately
            ...
            if system == 'Windows':
                ...
                with tempfile.NamedTemporaryFile(mode='w', suffix='.bat', delete=False, encoding='utf-8') as bat:
                    ...
                    bat.write(f'set PYTHONUNBUFFERED=1\n')
                    bat.write(f'{python_cmd} "{filepath}"\n')
                    bat.write(f'pause\n')
                    bat_path = bat.name
                
                # Run the batch file with clean environment and new process group
                ...
                # macOS - open in Terminal.app with proper escaping
                # Escape single quotes in the filepath for shell
                escaped_path = filepath.replace("'", "'\\''")
                # Build the shell command using only single quotes to avoid AppleScript conflicts
                shell_cmd = f"export PYTHONUNBUFFERED=1; {python_cmd} '{escaped_path}'; echo; echo 'Press any key to close...'; read -n 1"
                # Escape backslashes and double quotes for AppleScript
                escaped_cmd = shell_cmd.replace('\\', '\\\\').replace('"', '\\"')
                applescript = f"tell application \"Terminal\" to do script \"{escaped_cmd}\""
                subprocess.Popen(['osascript', '-e', applescript], env=clean_env)
                ...
                for term in terminals:
                    try:
                        bash_cmd = f'export PYTHONUNBUFFERED=1; {python_cmd} "{filepath}"; echo; echo "Press Enter to close..."; read'
