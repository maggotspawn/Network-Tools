    ...
    def run_selected_in_console(self):
        ...
        try:
            ...
            if system == 'Windows':
                ...
                # Create a diagnostic wrapper script
                with tempfile.NamedTemporaryFile(mode='w', suffix='.sh', delete=False, encoding='utf-8') as sh:
                    sh.write('#!/bin/bash\n')
                    sh.write('echo "Editor of Death - Python Script Runner"\n')
                    sh.write('echo "========================================"\n')
                    sh.write('echo\n')
                    sh.write(f'echo "Python command: {python_cmd}"\n')
                    sh.write(f'echo "Script path: {filepath}"\n')
                    sh.write('echo\n')
                    sh.write(f'if [ -f "{filepath}" ]; then\n')
                    sh.write(f'    echo "File exists - checking contents:"\n')
                    sh.write(f'    echo "File size: $(stat -f%z "{filepath}" 2>/dev/null || stat -c%s "{filepath}" 2>/dev/null) bytes"\n')
                    sh.write(f'    echo "First 3 lines:"\n')
                    sh.write(f'    head -n 3 "{filepath}"\n')
                    sh.write(f'    echo\n')
                    sh.write(f'    echo "--- OUTPUT START ---"\n')
                    sh.write(f'    {python_cmd} "{filepath}"\n')
                    sh.write(f'    EXIT_CODE=$?\n')
                    sh.write(f'    echo "--- OUTPUT END ---"\n')
                    sh.write(f'    echo\n')
                    sh.write(f'    if [ $EXIT_CODE -eq 0 ]; then\n')
                    sh.write(f'        echo "Script completed successfully (exit code: 0)"\n')
                    sh.write(f'    else\n')
                    sh.write(f'        echo "Script exited with error code: $EXIT_CODE"\n')
                    sh.write(f'    fi\n')
                    sh.write(f'else\n')
                    sh.write(f'    echo "ERROR: File not found: {filepath}"\n')
                    sh.write(f'    echo "Current directory: $(pwd)"\n')
                    sh.write(f'    echo "Looking for: {filepath}"\n')
                    sh.write(f'fi\n')
                    sh.write('echo\n')
                    sh.write('echo "Press Enter to close..."\n')
                    sh.write('read\n')
                    sh_path = sh.name
                ...
                terminals = [
                    ...
                    ['xfce4-terminal', '-e', 'bash', sh_path],
                    ['xterm', '-hold', '-e', 'bash', sh_path],
                    ['qterminal', '-e', 'bash', sh_path],
                    ['mate-terminal', '-e', f'bash {sh_path}'],
                    ['lxterminal', '-e', f'bash {sh_path}']
                ]
